...
export const updateProduct = actionClient.inputSchema(productUpdateSchema).action( async (values) => {
...
export const deleteProduct = actionClient.inputSchema(getProductByList).action(async (values) => {
    const { id } = values.parsedInput
    if(!ObjectId.isValid(id)) {
        return null
    }
